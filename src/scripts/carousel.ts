/* ============================================================
   ...
   Usage (from a page <script>):
     ...
     initCarousels({ feature: 3, voice: 6, honesty: 3 });
   ...
   count. Markup expectations per section:
     ...
     - prev/next controls carrying data-carousel="<name>":
         .slider-btn--prev / .slider-btn--next
         (or .reviews__slider-prev / .reviews__slider-next)
     - <div class="carousel-dots" data-carousel="<name>"> for dots

   Horizontal pixel deltas are divided by the section's render scale
   (read from the `.screen` element's CSS transform matrix, which is
   driven by the CSS-only `--scale`) so drag distance tracks the
   finger in the scaled design space.
   ============================================================ */
...
export function initCarousels(config: Record<string, number>): void {
  ...
  /* ============================================
     Scale factor helper — reads the computed transform
     matrix of the section's .screen (set by CSS --scale).
     ============================================ */
  function getScale(name: string): number {
    ...
    if (!screen) return 1;
    const matrix = getComputedStyle(screen).transform;
